...
new class extends UnitTestCase {
  ...
  protected test () {
    ...
    it('plays the previous song', async () => {
      ...
      await this.user.click(screen.getByRole('button', { name: 'Ecouter le sermon précédent' }))
    ...
    it('plays the next song', async () => {
      ...
      await this.user.click(screen.getByRole('button', { name: 'Ecouter le sermon suivant' }))
