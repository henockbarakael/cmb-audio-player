...
new class extends UnitTestCase {
  protected test () {
    ...
    it('submits the settings form', async () => {
      ...
      await this.type(screen.getByLabelText('Chemin des médias'), '/media')
    ...
    it('confirms upon Chemin des médias change', async () => {
      const updateMock = this.mock(settingStore, 'update')
      const goMock = this.mock(this.router, 'go')
      ...
      await this.type(screen.getByLabelText('Chemin des médias'), '/new')
      await this.user.click(screen.getByRole('button', { name: 'Scan' }))
