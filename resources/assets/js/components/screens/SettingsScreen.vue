<template>
  <section id="settingsWrapper">
    ...
    <form class="main-scroll-wrap" @submit.prevent="confirmThenSave">
      <div class="form-row">
        <label for="inputSettingsPath">Chemin des médias</label>

        <p id="mediaPathHelp" class="help">
          The <em>absolute</em> chemin d'accès au répertoire du serveur contenant votre média.
          CMB Audio Player recherchera les prédications dans ce répertoire et en extraira toutes les informations disponibles.<br>
          La numérisation peut prendre un certain temps, surtout si vous avez beaucoup de prédications, alors soyez patient.
        </p>
...
const shouldWarn = computed(() => {
  // Warn the user if the Chemin des médias is not empty and about to change.
...
const confirmThenSave = async () => {
  if (shouldWarn.value) {
    await showConfirmDialog('Changing the Chemin des médias will essentially remove all existing data – songs, artists, \
